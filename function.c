...
if(operand){
   if (operand[0] == '#') { // a number
           if (!isNumber(operand+1))  return NULL;//!
        return strdup(IMMEDIATE_ADDRESS);
    } else if (findLabbel(operand) +1) { // label
        return strdup(DIRECT_ADDRESS);
    } else if (findExtern(operand)) { // external label
        return strdup(DIRECT_ADDRESS);
    } else if ((isRegister(operand)+1)&&(operand[0] == '*')) { // A pointer to a register
        return strdup(INDIRECT_HOARD_ADDRESS);
    } else if (isRegister(operand)+1) { // A register
        return strdup(DIRECT_HOARD_ADDRESS);
    }else{//label no definition yet
	return strdup(DIRECT_ADDRESS);
    }
}
        return strdup(ZERO);
...
char *miunOperand(const char *operand, int firstOperand) {
    ...
    if (operand[0] == '#') { // a number
        ...
        strcpy(are, EXTERNAL);
    } else if (isRegister(operand)+1) { // A pointer to a register
