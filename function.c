...
char *findsMethodOfAddressing(const char *operand)
{
    ...
    else if (findExtern(operand) != NULL)
    { // external label
        return strdup(DIRECT_ADDRESS);
    }
    else if (operand[0] == '*')
...
char *miunOperation(int num, const char *operand1, const char *operand2)
{
    ...
    else
    {
        ...
        if (operand2 == NULL)
        { // own operand - second group
            strcpy(addressingMethod2, "0000");
        }
        else
        { // two operands - first group
...
int correctCommas(char *line)
{
    ...
    char *num = NULL;
    int inComma = 1; // if no comma attached after =0 else !=0
    while (token != NULL)
    { // check numbers correct
        num = strtok(token, ",");
        if (num)
        { // זה לא היה פסיק
            if (inComma == 1)
            ...
            { // too much comma
                fprintf(stderr, "Error: Sequence of commas.");
                return 0;
            }
            else
            { // number not after comma
                fprintf(stderr, "Error: Comma is missing.");
                return 0;
            }
        }
        else if (inComma || strlen(token) > 1)
        { // comma after comma
            fprintf(stderr, "Error: Sequence of commas.");
            return 0;
        }
